...
const buildEndpoint = (rpc: string): string => {
  if (!ENV.forgeApiUrl) {
    throw new TRPCError({
      ...
      message: "Heartbeat service URL is not configured (EXTERNAL_SERVICE_API_URL).",
    });
  }
  if (!ENV.forgeApiKey) {
    throw new TRPCError({
      code: "INTERNAL_SERVER_ERROR",
      message: "Heartbeat service API key is not configured (EXTERNAL_SERVICE_API_KEY).",
    });
  }
 ...
 * owner-only via the sandbox CLI (`external-heartbeat list --user-id <uid>`).
 */
